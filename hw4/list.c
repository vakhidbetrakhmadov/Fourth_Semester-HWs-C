...
static int readChNotWSpace(int fd,char* buffer,int bufferSize,coord_t* coordinatesArr,int startAt,int bytes2Read,int* row,int* col,int *linesSearcedInFile);
...
int findAllStringsInFile(const char* fileName, const char* string,FILE* outputp,int* linesSearcedInFile)
{
	int errnoFirst = errno;
	int inputfd;
	int closeValue;
	char* buffer = NULL;
	int bufferSize = 0;
	...
	if(fileName == NULL || string == NULL || outputp == NULL || linesSearcedInFile == NULL)
		return -1;

	*linesSearcedInFile = 0;
	...
	charsRead = readChNotWSpace(inputfd,buffer,bufferSize,coordinatesArr,1,bufferSize-1,&row,&col,linesSearcedInFile);
	if(charsRead == -1)
	{
		foundNumber = -1;
		errnoFirst = errno;
	...
	else if(charsRead == bufferSize-1)
	{
		while(!flag)
		{
			...
			if(!flag)
			{
				...
				charsRead = readChNotWSpace(inputfd,buffer,bufferSize,coordinatesArr,bufferSize-1,1,&row,&col,linesSearcedInFile);
			}
...
/* -1 on error, 0 on EOF, > 0 (char read) on success*/
/* puts '\0' at the end, bytes2Read must be <= (bufferSize-1) , size of coordinatesArr must be >= (bufferSize-1), startAt <= (bufferSize-1)*/
int readChNotWSpace(int fd,char* buffer,int bufferSize,coord_t* coordinatesArr,int startAt,int bytes2Read,int* row,int* col,int* linesSearcedInFile)
{
...
int outputTo(FILE* outputp, const coord_t* coordinate,const char* fileName,const char* string)
{
	...
	if(fprintf(outputp, "%d - %d %s: [%d, %d] %s first character is found. \n",getpid(),(int)pthread_self(),fileName,coordinate->row,coordinate->col,string) < 0 )
		return -1;
