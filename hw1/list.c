...
static int outputTo(FILE* outputp, const coord_t* coordinate);
...
int findAllStringsInFile(const char* fileName, const char* string,FILE* outputp)
{
	...
	else if(charsRead == bufferSize-1)
	{
		while(!flag)
		{
			...
			{	
				++foundNumber;
				if(outputTo(outputp,coordinatesArr) == -1)
				{	
					flag = 1;
					foundNumber = -1;
				}
...
int outputTo(FILE* outputp, const coord_t* coordinate)
{
	if(coordinate == NULL || outputp == NULL)
		return -1;

	fprintf(outputp, "[%d, %d] konumunda ilk karakter bulundu.\n",coordinate->row,coordinate->col);
